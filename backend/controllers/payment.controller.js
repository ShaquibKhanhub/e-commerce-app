...
export const createCheckoutSession = async (req, res) => {
  try {
    const { products, couponCode } = await req.body;
    ...
    if (session.payment_status === "paid") {
      if (session.metadata.couponCode) {
        await Coupon.findOneAndUpdate(
          ...
          { isActive: false } //user can use coupon only once
        );
      }
      // create a new Order
      const products = JSON.parse(session.metadata.products); //parse the products array from the metadata into an array of objects
